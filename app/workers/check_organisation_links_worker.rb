...
class CheckOrganisationLinksWorker
  ...
  def perform(organisation_id)
    GovukStatsd.time('link-checking-debug.check-organisation-links-worker') do
      ...
      Rails.logger.info("[link-checking-debug][org_#{organisation_id}][job_#{self.jid}]: Requesting link checks for #{editions.count}")
      ...
      Rails.logger.info("[link-checking-debug][org_#{organisation_id}][job_#{self.jid}]: Done requesting link checks for #{editions.count}, ignored #{ignored} of them")
    end
  end
