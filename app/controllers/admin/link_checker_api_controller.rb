class Admin::LinkCheckerApiController < ApplicationController
  ...
  def callback
    GovukStatsd.time('link-checking-debug.link-checker-callback') do
      Rails.logger.info("[link-checking-debug][batch_#{params[:id]}]: Updating link report")
      ...
      Rails.logger.info("[link-checking-debug][batch_#{params[:id]}]: Done updating link report")
    end
    head :no_content
  end
